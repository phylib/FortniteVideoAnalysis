...
def locate_map_scale(image):
    ...
    offsetX = image.shape[1] // 4 * 3
    offsetY = image.shape[0] // 2.5
...
# File 2: per Video_RoundId:
def write_round_wo_placment(outfile, phase, x_pos, y_pos, distance, video_time, matching_val):
    #print("write_round_wo_placment entered")
    outfile = open(outfile.name, "a")
    outfile.write(
        "{:1d},{:4d},{:4d},{:6.3f},{:s},{:6.3f}\n".format(phase, x_pos, y_pos, distance, video_time, matching_val))
    outfile.close()
    return None
...
if __name__ == '__main__':
    ...
    win_mask = cv.resize(win_mask, (710, 250), interpolation=cv.INTER_CUBIC)
            ...
            # skip frame if scale is known but video size changed
                ...
                if not waiting4newround:
                    ...
                    if not os.path.exists(output_round + rnd_id + ".csv"):
                        ...
                        outfile_round.write("phase,x_pos,y_pos,distance,video_time,matching_val\n")
                        outfile_round.close()
            # Scale is known
            else:
                ...
                region, x1, y1 = submatrix_video(frameV, (final_map_location.left, final_map_location.bottom),
                                                 (final_map_location.right-((final_map_location.right-final_map_location.left)//2),
                                                  final_map_location.bottom + text_region_h))

                #current_phase, max_val, max_loc = phase_find_best_match(region, phase_icons, mask_icon)
                current_phase, max_val, max_loc = phase_find_best_match(region, phase_icons, mask_icon)
                phase_max_val = max_val
                ...
                if not waiting4newround:
                    if current_phase <= 0:
                        ...
                        # check if player won
                            ...
                            region_, x1_, y1_ = submatrix_video(frameV, (550, 80), (500 + 850, 80 + 270)) # 710, 250
                            ...
                            #TODO change to region_
                            win, max_val_, max_loc_ = phase_find_best_match(region_, [win_mask], None)
