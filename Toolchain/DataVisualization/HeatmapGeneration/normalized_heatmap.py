...
class Fields:
    ...
    #phase, x_pos, y_pos, distance, video_time, matching_val = range(0, 6)
    phase, x_pos, y_pos, distance, video_time, gliding, matching_val_map, matching_val_phase, omitFrame = range(0, 9)


class Fields_file_list:
    # video, roundId, starttime, endtime, won, place, kills, playtime, tracename, groupsize = range(0, 10)
    video, roundId, starttime, endtime, won, place, kills, playtime, tracename, mapUiPositionAndScale = range(0, 10)

mapsize = 2000
heatmap_start = numpy.zeros((mapsize, mapsize))
heatmap_all = numpy.zeros((mapsize, mapsize))
heatmap_end = numpy.zeros((mapsize, mapsize))
...
max_landed_dist = 5
# 16.40116.401 max distance on boat
...
def ingest_file_content(file_path):
    ...
    with open(file_path, 'r') as file:
        ...
        for line_index in range(1, len(lines)):  # skip over header
            ...
            # check if frame was omited
            if int(parts[Fields.omitFrame]) == 0 and float(parts[Fields.matching_val_map]) >= 0.35:

                # find the first 10 values under max_movement_distance to find estimated landing spot
                if not landed:

                    if dist.__len__() < landed_time_check:

                        # add location to list
                        dist.append((float(parts[Fields.distance]), int(parts[Fields.x_pos]), int(parts[Fields.y_pos]),
                                     round_time_sec))
                        # check if enough entries are in the list and remove oldest if entry is 30 ago
                        if dist.__len__() > 30:
                            dist = dist[-30:]

                        # if gap between recognised bars is longer than 5 sec. reset map_locations
                        if len(dist) >= 2 and (dist[-1][3] - dist[-2][3]) > 5:
                            dist = dist[-1:]

                    # if list has enough entries check if gliding was detected consitantly
                    '''
                    #if True:
                    if dist.count("","","1","") > 20:
                        landed = True
                    ...
                    # Old Idendification if player Landed
                    # add location to list if value is slow enough to not be flight else reset list
                    if dist.__len__() < landed_time_check:
                        if 0 <= float(parts[Fields.distance]) <= max_landed_dist:
                    ...
                    if distance >= (dist_max + last_value_false * dist_max) or distance <= dist_min:
                        if distance >= (max_landed_dist + last_value_false * max_landed_dist) or distance < 0:
...
parser.add_argument("-o", "--output", help="Output folder and files name.", default="heatmap/")
parser.add_argument("-t", "--time", help="Start and end interval that's mapped (in seconds)", type=int, default=5)
parser.add_argument("-x", "--max", help="Max distance a player moves per sec", type=float, default=25)
parser.add_argument("-n", "--min", help="Min distance a player moves per sec (lowest value = 0)", type=float, default=1)
parser.add_argument("-w", "--won", help="filter if match was won or lost", type=str2bool, default=None)
...
with open(file_list, 'r') as files:
    for line in files.readlines()[1:]:  # skip over header
        parts = line.split(',')   # \t or ,
        # TODO find another indicator? time too long
...
map_path = 'Chapter_2_Season_1_Minimap_2000x2000.png'
